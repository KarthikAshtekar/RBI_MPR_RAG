...
from .final_evaluation import file_sha, make_checksum_manifest, stable_json_hash, write_csv, write_json
...
def actual_key_values_serialized(value: Any) -> bool:
...
def expected_case_lookup(root: Path = ROOT, split: str = "dev") -> dict[str, dict[str, Any]]:
...
def _score_record(score: float | None, *, success: bool = True, applicable: bool = True, method: str = "deterministic_heuristic", error_type: str | None = None, error_message: str | None = None) -> dict[str, Any]:
    return {
        "score": score,
        "success": success,
    ...
    for row in generation_rows:
        ...
        if not row.get("generation_success"):
            ...
            for name in METRIC_NAMES:
                metrics[name] = _score_record(None, success=False, error_type="generation_failed", error_message=row.get("generation_error_message"))
        else:
